...
## Purpose
Two Playwright specs (`a11y-marketing`, `golden-path-smoke`) plus the additions from Missions 04/13/14 still leave the deepest product flows e2e-untested: full import→submit against fixtures, recovery/checkpoint paths, document studio cycles, settings/policy changes taking effect. **Mission 17** defers **fixture-backed generate → lock → regen → download** — `document-studio.spec.ts` only asserts sub-nav, redirect, and chrome without live API. Mission 13 defers the **keyboard-only golden path** (import → batch → run console → checkpoint → submit) here — axe + palette/tab keyboard specs exist in `a11y-app.spec.ts` but not the full fixture journey. **Mission 15** defers **checkpoint resolve via UI** and **live SSE reconnect** e2e (hermetic `responsive-smoke` only asserts work-surface mount without fixture API) — the core journey spec here must cover resolve + stream reconnect against fixtures. **Mission 16** defers **fixture-backed batch preview** (included/excluded jobs) and **import → list → enqueue** — `discover-journey.spec.ts` only asserts page copy/chrome without live API. This mission builds the e2e suite that lets every future change be trusted without a manual walkthrough.
