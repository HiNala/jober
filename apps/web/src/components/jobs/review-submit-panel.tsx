...
import { ApproveSendBar } from "@/components/design-system/approve-send-bar";
...
export function ReviewSubmitPanel({ jobTargetId, onEditField }: ReviewSubmitPanelProps) {
  ...
  const firstEditableField = review.fill_diffs[0]?.field_key;
  ...
  return (
    ...
    >
      ...
      <ApproveSendBar
        className="-mx-0 rounded-b-lg border-border/60 bg-card/95"
        approveLabel="Approve & submit"
        editLabel="Edit"
        pauseLabel="Pause"
        onApprove={() => submitMutation.mutate(review.run_id)}
        onEdit={
          onEditField
            ? () => onEditField(firstEditableField ?? review.fill_diffs[0]?.field_key ?? "")
            : undefined
        }
        onPause={() => skipMutation.mutate(review.run_id)}
        approveDisabled={!ready}
        approvePending={submitMutation.isPending}
        pauseDisabled={skipMutation.isPending}
      />
    </section>
  );
}
